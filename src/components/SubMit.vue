          ...
          <div class="box-inner js-checkout-address-panel ">
            <div class="address-common-table js-multiple-address-panel">
              <ul class="address-item-list clear js-address-item-list">
                ...
                <li @click="$store.commit('addrassShow')" class="add-address-item js-add-address">
                  <p>使用新地址</p>
                </li>
              </ul>
            </div>
          ...
          <div class="invoice-detail"> 发票抬头：
            <div class="radio-box">
              <label>
                ...
                <span @click="$store.commit('oPisInvoice')"
                      class="blue-radio"
                     :class="$store.state.isInvoice?'':'blue-radio-on'"><a></a></span>  个人
              </label>
              <label>
                <input type="radio" class="hide">
                <span @click="$store.commit('oPisInvoice')"
                      :class="$store.state.isInvoice?'blue-radio-on':''"
                      class="blue-radio"><a></a></span>  单位
              </label>
            </div>
      ...
      <div class="gray-box">
        ...
        <div class="box-inner ui-goods-cart">
          ...
          <div class="cart-table">
            <div class="cart-group js-cart-group">
              <div class="cart-items" v-for="itte in goodShop" :key="itte.id">
                <div class="items-thumb">
                  <a href="javascript:;" target="_blank"><img :src="itte.ali_image"></a>
                </div>
                <div class="name hide-row">
                  <div class="name-cell">
                    <a href="javascript:;" title="坚果 Pro 钢化玻璃手感膜 开孔 (后壳用)（黑色）" target="_blank">{{itte.title}} (后壳用)（{{itte.spec_json.show_name}}）</a>
                  </div>
                </div>
                <div class="subtotal">
                  <div class="subtotal-cell">  ¥ {{itte.n*itte.price}}  </div>
                </div>
                <div class="goods-num">{{itte.n}}</div>
                <div class="price">¥ {{itte.price}}</div>
              </div>
            </div>
          </div>
        ...
        <div class="box-inner">
          <div class="order-discount-line">
            <p> 商品总计： <span>¥ {{$store.getters.sumPriceFn.toFixed(2)}}</span> </p>
        ...
        <div class="box-inner">
          <div class="last-payment clear">
            <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout"> <router-link to='/succeed' >提交订单</router-link> </span> <span class="prices fn-right">应付金额： <em>¥ {{$store.getters.sumPriceFn.toFixed(2)}}</em></span>
          </div>
        </div>
      </div>
    </div>
...
<script>
  import "@/assets/css/checkout.css"//引入css样式
  ...
  export default{
    ...
    computed:{
      goodShop(){//获取商品数据
        var arr=this.$store.state.commodNum.filter(item=>item.iscommodity);
        return this.$store.state.commodNum = arr
      }
    }
  }
</script>
